...
.. code-block:: bash
   ...
   docker run --detach -v dell_firmware:/dell_firmware --name dell-drm --network host --restart always rockylinux/rockylinux:9 sleep infinity

Copy in, and then run the installer:

.. code-block:: bash

   curl -O https://dl.dell.com/FOLDER13247371M/1/DRMInstaller_3.4.9.1169.bin -H 'User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/110.0'
   docker cp DRMInstaller_3.4.9.1169.bin dell-drm:/root
   docker exec -it dell-drm bash
   cd /root
   chmod +x DRMInstaller_3.4.9.1169.bin
   ./DRMInstaller_3.4.9.1169.bin
